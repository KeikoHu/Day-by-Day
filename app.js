// =============================
// Day by Day – Final JS with Delete & Difficulty Buttons
// =============================
...
function renderCharts(){
  ...
  for(let w=-5; w<=0; w++){
   ...
   // -----------------------------
// Historical Charts (last 6 weeks)
// -----------------------------
if(histLine) histLine.destroy();
...
addHabitBtn.onclick = ()=>{
  ...
  if(!name) return;
  // Set default emoji and difficulty
  const emoji = "🔥";
  const difficulty = "easy";
  state.habits.push({id:Date.now().toString(),name,emoji,difficulty});
  save(); render();
};
...
resetWeekBtn.onclick = ()=>{
  ...
  getWeekKeys().forEach(k=>{
    state.habits.forEach(h=>delete state.completions[h.id]?.[k]);
  });
  save(); render();
};
